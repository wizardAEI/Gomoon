...
export default function (props) {
    ...
    return (<div class="relative mx-2 my-4 flex flex-col gap-2 rounded-2xl  bg-dark p-4 duration-150">
      <span>记忆名称</span>
      <input type="text" class="w-full" value={m().name} onChange={(e) => setField('name', e.currentTarget.value)} placeholder="记忆胶囊名称"/>
      <span>介绍</span>
      <input type="text" value={m().introduce ?? ''} onChange={(e) => setField('introduce', e.currentTarget.value)} placeholder="可不填"/>
      <div class="my-1 mb-3">
        <div class="mb-2 flex items-center justify-between">
          <span class="flex items-center gap-1">
            记忆片段（支持Md
            <QuestionMention content={<span class="text-xs">
                  <a class="text-xs" href="https://gomoon.top/guide/%E5%BF%AB%E9%80%9F%E4%BD%BF%E7%94%A8%E8%AE%B0%E5%BF%86%E8%83%B6%E5%9B%8A">
                    将其他格式文件转为markdown文件
                  </a>
                </span>}/>
            ）
          ...
          <div>
            <Switch size="sm" label={<span class="text-xs">
                  大模型优化
                  <span class="text-xs text-gray"> (当前模型: {userData.selectedModel})</span>
                </span>} checked={useLLM()} onCheckedChange={() => {
            setUseLLM(!useLLM());
        }}/>
          </div>
        </div>
        <For each={m().fragment}>
          {(file) => (<div class="mt-2 flex select-none justify-between">
              <div class="flex gap-1">
                <BaseFileIcon height={20} width={20}/>
                {file.name}
              </div>
              <DoubleConfirm label="确认删除" position="-right-2 top-3" 
        // eslint-disable-next-line solid/reactivity
        onConfirm={async () => {
                ...
                if (res.suc) {
                    setField('fragment', m().fragment.filter((f) => f.name !== file.name));
                }
            }}>
        ...
        <label for="file" class="cursor-pointer">
          <div class="group/add mt-2 flex w-full cursor-pointer items-center justify-center gap-1 rounded-md border-dashed border-gray py-1 hover:border-active">
            ...
            <Plus class="text-gray duration-100 group-hover/add:text-active" height={20} width={20}/>
          </div>
          <input id="file" type="file" class="hidden" accept=".md" multiple={true} onChange={async (e) => {
