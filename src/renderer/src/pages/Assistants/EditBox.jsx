...
function Field(props) {
    return (<div class="my-2 flex flex-col gap-2">
...
export default function EditBox(props) {
    ...
    return (<div class="relative mx-2 my-4 rounded-2xl bg-dark p-4 duration-150">
      <div class="mb-2 flex items-center gap-4 md:justify-center">
        <div class="relative flex cursor-pointer" onClick={() => {
        ...
        <div class="flex flex-col gap-2">
          ...
          <input type="text" value={a().name} onChange={(e) => setField('name', e.currentTarget.value)} placeholder="助手名称"/>
        </div>
      </div>
      <div class="item-center relative mb-2 mt-3 flex w-full justify-between md:justify-center">
        ...
        <div class="max-w-[180px] pl-4">
          <Select defaultValue={a().matchModel || 'current'} options={[
            {
                value: 'current',
                label: '跟随当前模型'
            },
      ...
      <Field title="介绍">
        {/* <input
          ...
          placeholder=""
        /> */}
        <textarea class="text-sm" rows={3} value={a().introduce ?? ''} onChange={(e) => setField('introduce', e.currentTarget.value)} placeholder="可不填"/>
      </Field>
      <Field title="提示（Prompt）">
        <textarea class="text-sm" rows={4} value={a().prompt} onChange={(e) => setField('prompt', e.currentTarget.value)} placeholder="告诉助手要做什么吧"/>
      </Field>
