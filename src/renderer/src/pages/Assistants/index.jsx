...
import { assistants, assistantsStatus, createNewAssistant, deleteAssistant, getCurrentAssistantForAnswer, getCurrentAssistantForChat, onCancelEditAssistant, onEditAssistant, saveAssistant } from '@renderer/store/assistants';
    ...
    return (<div class="mx-auto w-full overflow-hidden lg:max-w-4xl">
      <div class="mb-5 animate-scale-down-entrance select-none p-2">
        <div class="mx-2 my-4 flex gap-2">
          <div class="group/create relative flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-2xl bg-dark py-4" onClick={() => {
            createAssistant();
        }}>
          ...
          <div class="group/create relative flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-2xl bg-dark py-4" onClick={() => {
            ...
            <BrowserIcon height={24} width={24} class="cursor-pointer text-gray duration-100 group-hover/create:text-active"/>
            <span class="text-base">探索更多助手</span>
          </div>
        </div>

        <For each={assistants.filter((a) => a.type === type)}>
          {(a) => (<Show when={assistantsStatus[a.id] === 'saved'} fallback={<EditBox assistant={a} onCancel={() => {
                    onCancelEditAssistant(a.id);
                }} onSave={saveAssistant}/>}>
              <div class="relative mx-2 my-4 flex flex-col gap-3 rounded-2xl border-2 border-solid border-transparent bg-dark p-3 duration-150 hover:border-active" onClick={async () => {
                switch (type) {
                    case 'ans':
                ...
                <Show when={getCurrentAssistantForAnswer()?.id === a.id ||
                getCurrentAssistantForChat()?.id === a.id}>
                  <InuseIcon width={34} height={34} class="absolute -right-[2px] -top-[2px] text-active"/>
                </Show>
                ...
                e.stopPropagation();
                    ...
                    <DoubleConfirm label="确认删除" position="-right-2 top-3" onConfirm={() => deleteAssistant(a.id)} preConfirm={() => {
                const canDel = a.id !== getCurrentAssistantForAnswer()?.id &&
