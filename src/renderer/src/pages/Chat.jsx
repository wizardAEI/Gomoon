...
import { msgs, pushMsg, pushGeneratingStatus, msgStatus, editMsg, reActiveGeneratingStatus, genMsg, removeMsg, restoreMsgs } from '../store/chat';
const scrollToBottom = (el, index) => {
...
export default function Chat() {
    ...
    const [previousMsg, setPreviousMsg] = createSignal({ content: '', id: '', state: 'complete' });
    ...
    return (<div class="chat-container relative flex h-[calc(100vh-124px)] flex-col overflow-auto pb-24 pt-10">
      <Show when={msgs.length} fallback={<>
        ...
        <Show when={msgStatus.generatingList.length === 0}>
          <Capsule type="chat" botName={getCurrentAssistantForChat().name} seed={getCurrentAssistantForChat().avatar || getCurrentAssistantForChat().id.slice(-5)}/>
        </Show>
        <GlobalSearch />
        ...
        <Show when={msg.content} fallback={<div ref={(el) => scrollToBottom(el, index())} id={msg.id} class={'mb-1 flex ' + (msg.role === 'human' ? 'human ml-4 justify-end' : 'ai mr-4')}>
                    <Message isEmpty editing={msg.id === editId()} id={msg.id} content="......" type={msg.role} botName={getCurrentAssistantForChat().name}/>
                  </div>}>
                <div ref={(el) => scrollToBottom(el, index())} class={`relative mb-1 flex max-w-[calc(100%-16px)] ${msg.role === 'human' ? 'human ml-4 justify-end' : 'ai mr-4'}`}>
                  <Message editing={msg.id === editId()} id={msg.id} content={msg.content} type={msg.role} botName={getCurrentAssistantForChat().name} onRemove={() => handleRemoveMsg(msg.id)}/>
                </div>
              </Show>)}
          </For>
        </div>
      ...
      <div class="fixed bottom-10 z-20 w-full px-4">
        <Input onInput={() => {
            cleanupForRestoreMsgs?.();
        }} showClearButton={previousMsg().state === 'complete'} onClear={() => {
    ...
    // 自动聚焦
    onMountHandler={(inputDiv) => {
            inputDiv.focus();
        }} 
    // 显示时自动聚焦
    autoFocusWhenShow isGenerating={msgStatus.generatingList.length > 0} type={msgs.find((msg) => msg.id === editId())?.role || 'human'}/>
      </div>
    </div>);
}
