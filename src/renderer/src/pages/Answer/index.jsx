...
export default function Answer() {
    ...
    onMount(() => {
        ...
        const removeListener = window.api.multiCopy((_, msg) => {
            setInputText(msg);
            setShowModal(true);
        });
    ...
    return (<div class="flex h-[calc(100vh-124px)] flex-col gap-4 overflow-auto pb-48 pt-10">
      <Show when={showModal()}>
        <SelectAssistantModal onConfirm={() => {
            setShowModal(false);
            genAns(inputText());
            setInputText('');
        }} onCancel={() => {
      ...
      <div class="mx-auto flex w-full flex-col gap-4 lg:max-w-4xl">
        <Show when={answerStore.question} fallback={<>
        ...
        <Show when={answerStore.answer}>
          <Show when={!ansStatus.isGenerating}>
            <Capsule type="ans" botName={getCurrentAssistantForAnswer().name} seed={getCurrentAssistantForAnswer().avatar || getCurrentAssistantForAnswer().id.slice(-5)}/>
          </Show>
          <Message content={answerStore.answer} type="ans" botName={getCurrentAssistantForAnswer().name}/>
        </Show>
      </div>
      ...
      <div class="fixed bottom-10 z-20 w-full px-4">
        <Input showClearButton disable={showModal()} send={genAns} 
    // 自动聚焦
    onMountHandler={(input) => {
            input.focus();
        }} 
    // onShow自动聚焦
    autoFocusWhenShow type="question"/>
      </div>
    </div>);
}
