...
// 预渲染一遍，不考虑响应式
export default function SpecialTypeContent(meta, max = 50, displayFull = false) {
    if (meta.type === 'file') {
        return (<span class="inline-flex items-start break-words pr-1">
        <BaseFileIcon class="shrink-0 grow-0 translate-y-[2px] pr-[2px] text-text2" width={17} height={17}/>
        <span class="text-text2">{decorateContent(meta.filename)}</span>
      </span>);
    }
    if (meta.type === 'url') {
        return (<div class="inline-flex items-start">
        <LinkIcon class="mr-[2px] shrink-0 grow-0 text-text-dark2 group-hover/link:text-active" width={14} height={20}/>
    ...
    if (meta.type === 'search') {
        return (<span class="inline-flex items-start break-words">
    ...
    if (meta.type === 'memo') {
        return (<span class="inline-flex items-start break-words">
        <CapsuleIcon class="shrink-0 grow-0 translate-y-[1px] pr-[2px] text-text2" width={17} height={17}/>
        <span class="text-text2">{decorateContent(meta.question, max)}</span>
      </span>);
    }
    if (meta.type === 'image') {
        ...
        <img onClick={() => {
            ...
            }} src={meta.value} class={'inline cursor-pointer rounded-sm ' + (displayFull ? 'max-h-14' : 'w-6')}/>
        <br />
      </span>);
    }
    return null;
