...
import { historyManager, clearHistory, copyHistory, histories, removeHistory, starHistory } from '@renderer/store/history';
...
export default function () {
    ...
    const filteredHistory = createMemo(() => {
        let arr = histories;
        if (selectType() === 'starred')
            arr = arr.filter((h) => h.starred);
        return arr.filter((a) => a.contents.find((c) => c.content.includes(searchText())));
    });
    return (<div class="relative flex h-full select-none flex-col px-3 pt-2">
      ...
      <div class="mx-auto mb-[10px] flex min-h-8 w-full items-center justify-between gap-2 px-1 lg:max-w-4xl">
        <Show when={!showSearchInput()} fallback={<div ref={(el) => {
                const fn = (e) => {
              ...
              <Search placeholder="搜索...（点击其他区域返回）" onChange={(v) => {
                setSearchText(v);
            }}/>
            </div>}>
          <SegmentedControl defaultValue={selectType()} options={[
            {
                label: '全部',
                value: 'all'
            },
          ...
          <div class="flex gap-2">
            ...
            toast
                ...
                if (res) {
                    ...
                    <ToolTip label={<div class={iconClass[0]}>
                          <ClearIcon width={22} height={22} class={iconClass[1]}/>
                        </div>} content="清除历史记录（非收藏）"/>
                  </div>
                </Show>
                <ToolTip label={<div class={iconClass[0]} onClick={() => setShowMoreBtn(false)}>
      ...
      <ScrollBox>
        <Show when={selectType() !== 'collection'} fallback={<Collection searchText={searchText()}/>}>
          <div class="w-full px-1 lg:max-w-4xl">
            <Show when={histories.length} fallback={<div class="relative m-auto flex h-40 w-full select-none flex-col items-center justify-center gap-3 rounded-lg bg-dark p-5 duration-150">
            ...
            return (<>
                    ...
                    (showMore() ? 'z-20' : '')}>
                        ...
                        <div class="absolute right-2 top-[2px]">
                          <Show fallback={<div class={iconClass[0]} onClick={() => setShowMore(true)}>
                                <MoreHIcon width={20} height={20} class={iconClass[1]}/>
                              </div>} when={showMore()}>
                            <div class="flex flex-col gap-1 rounded-md bg-dark px-2 py-1 shadow-center" ref={(el) => {
                    const fn = (e) => {
                        if (e.target && el.contains(e.target)) {
                            e.stopPropagation();
                            return;
                    ...
                    h.starred ? starHistory(h.id, false) : starHistory(h.id, true);
                                ...
                                <StarIcon width={20} height={20} class={`cursor-pointer ${h.starred ? 'text-active' : 'text-gray group-hover/btn:text-active/80'}`}/>
                    ...
                    removeHistory(h.id);
                      ...
                      <div class="group/history-box relative mb-3 flex w-full cursor-pointer flex-col gap-2 rounded-b-lg border-2 border-solid border-transparent bg-dark p-4 pt-2 duration-150 hover:border-active lg:max-w-4xl" onClick={() => {
                    ...
                    return (<div class="flex flex-col gap-1 break-words text-sm">
                                <div>
                                  <For each={meta}>
                                    {(m, index) => {
                            return m.type === 'text' ? ((index() === 0 ? map[c.role] || '我：' : '') +
                                decorateContent(m.content)) : (<>
                                          {index() === 0 && (<span class="mr-1">{map[c.role] || '我：'}</span>)}
