...
import type { Provider } from '@lib/models-config'
import { useToast } from '@renderer/components/ui/Toast'

export default function ModelEngineConfig() {
  const toast = useToast()
  const [loadingProviderId, setLoadingProviderId] = createSignal<string | null>(null)
  const [providerModels, setProviderModels] = createSignal<
    Record<string, { id: string; object?: string }[]>
  >({})
  ...
  const addModel = (providerId: string, modelId: string) => {
    ...
    if (!provider) return
    const exists = settingStore.models.enabledModels.some(
  ...
  const getProviderName = (id: string) =>
    settingStore.models.providers.find((p) => p.id === id)?.name ?? id
      ...
      >
        <div class="flex flex-col gap-3">
          <For each={settingStore.models.providers}>
            {(p) => (
              <div class="rounded-xl border border-gray/40 bg-dark-con/40 p-4 transition-colors hover:border-gray/60">
                <div class="mb-3 flex items-center justify-between">
                  <span class="font-medium text-text1">{p.name || '未命名'}</span>
                  <button
                    class="rounded px-2 py-0.5 text-xs text-text2 transition-colors hover:bg-danger/20 hover:text-danger"
                ...
                <div class="flex flex-col gap-2">
                  <EditInput
                    label="名称"
                    value={p.name}
                    onSave={(v) => updateProvider(p.id, { name: v.trim() })}
                  />
                  <EditInput
                    label="apiKey"
                    value={p.apiKey}
                    onSave={(v) => updateProvider(p.id, { apiKey: v.trim() })}
                  />
                  <EditInput
                    label="baseURL"
                    value={p.baseURL}
                    onSave={(v) => updateProvider(p.id, { baseURL: v.trim() })}
                  />
                </div>
                <button
                  class="mt-3 w-fit rounded-lg bg-active/90 px-3 py-1.5 text-sm font-medium text-white transition-opacity hover:bg-active disabled:opacity-50"
                  disabled={loadingProviderId() === p.id}
                  onClick={() => fetchModels(p)}
                >
                  {loadingProviderId() === p.id ? '获取中...' : '获取可用模型'}
                </button>
                {providerModels()[p.id]?.length ? (
                  <div class="mt-3 max-h-36 overflow-auto rounded-lg border border-gray/30 bg-dark/60 p-2">
                    <div class="mb-2 text-xs text-text2">点击添加至使用列表：</div>
                    <div class="flex flex-wrap gap-1.5">
                      <For each={providerModels()[p.id]}>
                        {(m) => (
                          <button
                            class="rounded-md bg-dark-con px-2.5 py-1 text-xs text-text1 transition-colors hover:bg-active hover:text-white"
                            onClick={() => addModel(p.id, m.id)}
                          >
                            {m.id}
                          </button>
                        )}
          ...
          <button
            class="flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray/50 px-4 py-3 text-sm text-text2 transition-colors hover:border-active hover:text-active"
            onClick={() =>
              addProvider({
                name: '新建提供商',
                apiKey: '',
      ...
      >
        <div class="flex flex-col gap-2">
          <For each={settingStore.models.enabledModels}>
            {(em) => (
              <div class="flex items-center justify-between gap-3 rounded-xl border border-gray/40 bg-dark-con/40 p-3 transition-colors hover:border-gray/60">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <div class="w-20">
                      <Slider
                        ...
                        percentage
                        onChange={(v) => updateEnabledModel(em.id, { temperature: v })}
                      />
                    </div>
                  </div>
                  <button
                    class="rounded px-2 py-0.5 text-xs text-text2 transition-colors hover:bg-danger/20 hover:text-danger"
                    onClick={() => removeEnabledModel(em.id)}
                  >
                    移除
                  </button>
                </div>
          ...
          {settingStore.models.enabledModels.length === 0 && (
            <div class="rounded-xl border border-dashed border-gray/40 py-8 text-center text-sm text-text2">
