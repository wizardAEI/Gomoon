...
let controller;
let ansID;
export function setGeneratingStatus(status) {
    setAnsStatus(produce((ansStatus) => {
        ansStatus.isGenerating = status;
    }));
}
export async function genAns(q) {
    ...
    try {
        ...
        if (!ansStatus.isGenerating)
            return;
        setAnswerStore('answer', (ans) => {
            return ans === initialContent ? ErrorDict(err) : ans + ErrorDict(err);
        });
        setGeneratingStatus(false);
    }
}
