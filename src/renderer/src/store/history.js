...
import { msgMeta, msgs, setMsgMeta, setMsgs } from './chat';
...
const [histories, setHistories] = createStore([]);
export async function loadHistories() {
    return window.api.getHistories().then(setHistories);
}
export async function starHistory(historyID, status) {
...
export async function copyHistory(historyID) {
