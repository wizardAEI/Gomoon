...
async function fetchBaiduResults(keyword) {
    ...
    const links = [];
    links.push(...data.feed.entry.map((item) => ({
        summary: item.abs,
        link: item.url
    })));
    return links.filter(({ summary }) => summary);
}
function processQuery(content) {
    const reg = /<(.+?)>/;
...
export async function searchByBaidu(question, logger) {
    ...
    while (times--) {
        ...
        processedContent = processQuery(res.content);
    ...
    `;
        try {
            const c = (await nonStreamingAssistant(prompt2)).content;
            if (c.includes('yes')) {
                valuableLinks.push(link.link);
            }
        }
        finally {
            logger(`收集链接中(${valuableLinks.length}/2)，${linkNum - links.length - valuableLinks.length}个无效链接`);
        }
    }
    ...
    logger(`解析链接中...`);
    const res = await Promise.all(valuableLinks.map(async (link) => {
        return {
