...
export function PopupContainer(props) {
    ...
    return (<div class={`absolute flex h-[26px] items-center gap-3 rounded-[10px] px-1 opacity-0 duration-200 group-hover:opacity-100 ${props.pos && posDict[props.pos]}`}>
      {props.children}
    </div>);
}
const btnClass = [
...
export function MsgPopupContents(props) {
    ...
    return (<>
                ...
                toast.modal((option) => {
                    ...
                    return (<div class="flex w-[80vw] max-w-xl flex-col gap-4 p-1">
                      ...
                      <div class="flex w-full flex-col gap-2">
                        <div>现有合集</div>
                        <Show when={collections.length} fallback={<div class="flex items-center justify-center rounded-md border-dashed border-gray p-4">
                            ...
                            <ScrollBox>
                              <div class="w-full py-1">
                                <For each={collections}>
                                  {(c) => {
                            return (<div class="py-[2px] pr-1">
                                        <div class={'flex cursor-pointer justify-between rounded-md px-2 py-1 hover:bg-active hover:text-text-active ' +
                                    (c.name === collectionName() &&
                                        'bg-active text-text-active')} onClick={() => {
                      ...
                      <div class="flex flex-col gap-2">
                        <div class="">新合集名称</div>
                        <Show when={addNewCollection()} fallback={<Button onClick={() => {
                                setCollectionName('');
      ...
      <Show when={props.type === 'ans'}>
        <ToolTip label={compWithTip((tip) => (<div class={btnClass[0]} // eslint-disable-next-line solid/reactivity
         onClick={async () => {
      ...
      <ToolTip label={<div class={btnClass[0]} onClick={props.onSpeak}>
            <SpeechIcon height={20} width={20} class={btnClass[1]}/>
          </div>} content="朗读"/>
      <Show when={props.type !== 'ans'}>
        <ToolTip label={<div class={btnClass[0]} onClick={() => {
...
export function MsgPopupForUser(props) {
    const [source] = createSignal('');
    const { copy } = useClipboard({ source });
    return (<PopupContainer pos="right">
      <ToolTip label={compWithTip((tip) => (<div class={btnClass[0]} onClick={() => {
                copy(props.content).then(() => tip('success', '复制成功！'));
            }}>
              <CopyIcon height={20} width={20} class={btnClass[1]}/>
            </div>), 'right')} content="复制到剪贴板"/>
      <ToolTip label={<div class={btnClass[0]} onClick={props.onRemove}>
...
export function MsgPopupForSpecialContent(props) {
    return (<Show when={props.type === 'human'}>
...
export function WithDrawal(props) {
    return (<PopupContainer pos={props.type === 'human' ? 'right' : 'left'}>
      <ToolTip label={<div class={btnClass[0]} onClick={() => {
...
export function Pause(props) {
    return (<PopupContainer pos="left">
      <div onClick={() => {
            if (props.id) {
                stopGenMsg(props.id);
            }
        ...
        <PauseIcon height={20} width={20} class="cursor-pointer text-gray-pro duration-100 group-hover/pause:text-active"/>
