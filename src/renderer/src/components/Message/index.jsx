...
export default function Message(props) {
    ...
    // FEAT: 是否显示小组件
    const showComps = createMemo(() => (props.type === 'ai' || props.type === 'ans') && !isGenerating());
    const showCompsByUser = createMemo(() => {
        if (props.id) {
            const genIndex = msgs.findIndex((msg) => msg.id === props.id) + 1;
            return (!msgStatus.generatingList.includes(msgs[genIndex]?.id || '') &&
                props.type === 'human' &&
    ...
    createEffect(() => {
        document.documentElement.style.setProperty('--gomoon-md-fontsize', `${settingStore.chatFontSize}px`);
    });
    return (<div class="group relative max-w-full">
      <div class={style[props.type] + ' relative mx-3 my-4 rounded-xl px-3 py-[10px]'}>
        ...
        <Show when={showComps() && !props.editing}>
          <MsgPopup type={props.type} id={props.id || ''} content={props.content} onSpeak={speakMd}/>
        </Show>
        <Show when={!props.editing} fallback={<Show when={props.id && !msgStatus.generatingList.includes(props.id)}>
              <WithDrawal type={props.type}/>
            </Show>}>
          <Show when={showCompsByUser()} fallback={<MsgPopupForSpecialContent type={props.type} onRemove={props.onRemove || (() => { })}/>}>
            <MsgPopupForUser type={props.type} id={props.id || ''} content={props.content} onRemove={props.onRemove || (() => { })}/>
          </Show>
        </Show>
        <For each={meta()}>
          {(m) => m.type === 'text' ? (<Md needSelectBtn showHtml={props.type === 'ai' || props.type === 'ans'} class={mdStyle[props.type]} content={m.content} onSpeak={speakText} isGenerating={isGenerating()}/>) : (SpecialTypeContent(m, mdStyle[props.type], {
            needSelectBtn: true
        }))}
        </For>
      </div>
