...
export default function ModelSelect(props: {
  ...
  const label = createMemo(() => {
    ...
    return (
      <span class={`max-w-32 truncate text-ellipsis ${isPlaceholder ? 'text-text2' : ''}`}>{text}</span>
    )
  })
  ...
  return (
    <div class="relative flex">
      ...
      <Show when={isOpen()}>
        <div
          class={`absolute top-full z-10 mt-2 flex flex-col rounded-md bg-dark-plus px-1 shadow-center ${
            props.position
