...
export default function CapitalIcon(props) {
    const [state, send] = useMachine(tooltip.machine({ id: createUniqueId(), openDelay: 200, closeDelay: 300 }));
    const api = createMemo(() => tooltip.connect(state, send, normalizeProps));
    const firstChat = props.content.charAt(0);
    return (<div>
      <button {...api().triggerProps} class={`flex items-center justify-center rounded-md border-0 p-0 text-[12px] font-bold leading-3 ${props.bg || 'bg-gray'}`} style={{
