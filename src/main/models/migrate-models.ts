...
function isOldModels(m: unknown): m is Record<string, any> {
  if (!m || typeof m !== 'object') return false
  const o = m as Record<string, unknown>
  return 'OpenAI' in o || 'providers' in o
}

function hasNewFormat(m: unknown): m is ModelsConfig {
  if (!m || typeof m !== 'object') return false
  const o = m as Record<string, unknown>
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function migrateModels(models: unknown): ModelsConfig {
  if (hasNewFormat(models)) return models as ModelsConfig
  if (!isOldModels(models)) return { providers: [], enabledModels: [] }

  const providers: Provider[] = []
  const enabledModels: EnabledModel[] = []

  const addProvider = (name: string, apiKey: string, baseURL: string) => {
  ...
  const addModel = (providerId: string, modelId: string, label?: string, temp = 0.3) => {
    enabledModels.push({
      id: ulid(),
      providerId,
      modelId,
  ...
  if (ollama?.address && ollama.model) {
    ...
    if (pid) {
      ...
      if (ollama.model1) addModel(pid, ollama.model1, 'Ollama 备用1', ollama.temperature ?? 0.3)
      if (ollama.model2) addModel(pid, ollama.model2, 'Ollama 备用2', ollama.temperature ?? 0.3)
    }
  }
  ...
  if (custom?.models?.length) {
    for (const m of custom.models) {
      if (m.apiKey && m.customModel) {
        ...
        if (pid) addModel(pid, m.customModel, m.customModel, m.temperature ?? 0.3)
      }
    }
  }
