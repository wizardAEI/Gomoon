...
export function setQuicklyWakeUp(keys) {
    /**
     * FEAT: 按键监听
     */
    globalShortcut.register(keys, () => {
        function showWindow() {
            const getSelected = async () => {
                ...
                const text = lastText === (clipboard.readText('clipboard') || '')
                    ? ''
...
export function updateSendHeaders(urls = []) {
    session.defaultSession.webRequest.onBeforeSendHeaders({ urls: cors.defaultURLs.concat(urls.map((url) => (url.endsWith('/*') ? url : url + '/*'))) }, cors.handler);
}
export function updateRespHeaders(urls = [], conf) {
    session.defaultSession.webRequest.onHeadersReceived({ urls: cors.defaultURLs.concat(urls.map((url) => (url.endsWith('/*') ? url : url + '/*'))) }, (details, callback) => {
...
export async function postMsgToMainWindow(msg) {
    return mainWindow?.webContents.send('post-message', msg);
}
export async function PostBuffToMainWindow(buff) {
