...
export async function callLLM(options) {
    ...
    await llm.streamInvoke(options.msgs.map((msg) => ({ role: msg.role, content: msg.content })), {
...
export async function lmInvoke(option) {
    const llmMap = await loadLMMapForNode(getLMConfig().models);
    ...
    if (option.system)
        msgs.unshift({ role: 'system', content: option.system });
    try {
