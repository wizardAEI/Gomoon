...
export function createTreeFromMarkdown(markdown, config) {
    const tree = [];
    const tokens = md.parse(markdown, {});
    let tokenIndex = 0;
    const isType = (tokenType) => {
        ...
        if (tokens[tokenIndex].type === 'table_open') {
            ...
            for (; tokenIndex < tokens.length; tokenIndex++) {
                ...
                if (token.type === 'tr_open') {
                    ...
                    if (token.type === 'th_open' && !isHeader)
                        headerNum++;
                }
...
async function getQuestionsByLM(total) {
    const content = (await lmInvoke({
...
export async function getChunkFromNodes(nodes, options) {
    const chunk = [];
    const { chunkSize = 1500, chunkOverlap = 2, useLLM = false, skipContent = false, skipOnlyTitleContent = true, nodesFrom } = options;
    const chunkTask = [];
    ...
    const dfs = (nodes, index, data) => {
        const node = nodes[index];
        const totalTitle = splice(data?.titleBefore ? data?.titleBefore : '', node.markup + ' ' + node.title);
        if (node.total.length < chunkSize) {
            ...
            const pushContent = async (contents) => {
                for (let i = 0; i < contents.length; i++) {
                    ...
                    if (!skipContent)
                        chunk[chunkIndex].indexes.push({ value: content });
                    if (useLLM) {
